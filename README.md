...
数据操作方法使用 mongoose 中 model 提供的方法来进行具体的增删改查，官方提供的方法支持回调函数，但是为了避免回调地狱以及提升代码可读性采用 then 的方式来处理 model 方法返回的数据
这里需要注意的是， model 方法支持的回调函数 默认第一个返回参数是错误信息，第二个返回参数是返回数据，而 then 默认第一个返回参数是返回数据，第二个参数是返回错误信息

##### /admin/product
GET方式访问，返回全部商品管理的全部可使用接口的描述信息
##### /admin/product/addItem 用于添加商品信息
GET方式访问，返回添加商品信息需要传递的参数及格式
POST方式访问，添加成功 返回数据包含
```
msg: '商品信息添加成功'
```
失败 返回数据包含
...
修改后没有找到数据 返回数据包含
```
msg: '已修改 x 条数据,但是查询修改后的数据失败'
```
修改成功并找到修改结果并返回 返回数据包含
```
msg: '提交修改命令成功，已修改 ${info.nModified} 条数据'
```
##### /admin/product/list 用于查询商品信息列表
GET方式访问，返回查询全部商品信息需要传递的参数及格式
POST方式访问，这个会有三种情况  
查询失败 返回数据包含
```
msg: '查询商品列表失败'
